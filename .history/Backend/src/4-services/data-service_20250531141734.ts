...
class DataService {
    ...
    public async deleteMeeting(_id: string): Promise<void> {
        const dbMeeting = MeetingModel.findByIdAndDelete(_id).exec();
        if (!dbMeeting) throw new ClientError(StatusCode.NotFound, `Meeting ${_id} does not exist.`);
    }
