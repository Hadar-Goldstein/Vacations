...
class DataService {
    ...
    public updateVacation(vacation: IVacationModel): Promise<IVacationModel> {
        const error = vacation.validateSync()
        if(error) throw new ClientError(StatusCode.BadRequest, error.message);
        const dbProduct = await VacationModel.findByIdAndUpdate(product._id, product, {returnOriginal: false}).exec();
        if (!dbProduct) throw new ClientError(StatusCode.NotFound, "Product does not exist");
        return dbProduct;
    }
